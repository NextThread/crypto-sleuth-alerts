...
import DetailedTechnicalAnalysis from '../components/DetailedTechnicalAnalysis';
...
import { AlertTriangle, BarChart2, LineChart, TrendingUp, Info, Sparkles, User } from 'lucide-react';
...
const Index = () => {
  ...
  const handleSymbolSelect = async (newSymbol: string) => {
    if (!user) {
      toast({
        ...
        description: "Please sign in to search for cryptocurrencies.",
        variant: "destructive",
      });
  ...
  const handleIntervalChange = (newInterval: TimeInterval) => {
    if (!user) {
      toast({
        title: "Authentication Required",
        description: "Please sign in to change chart intervals.",
        variant: "destructive",
      });
      setShowLoginAlert(true);
      return;
  ...
  const handleChartControlsChange = (newControls: ChartControlsState) => {
    if (!user) {
      toast({
        title: "Authentication Required",
        description: "Please sign in to customize chart controls.",
        variant: "destructive",
      });
      setShowLoginAlert(true);
      return;
  ...
  return (
    <Layout>
      <div className="max-w-7xl mx-auto">
        ...
        {showLoginAlert && (
          <Alert className="mb-6 border-primary/50 bg-primary/10 backdrop-blur-md animate-fade-in shadow-lg">
            <AlertTriangle className="h-4 w-4 text-primary" />
            <AlertDescription className="flex justify-between items-center">
              <div>
                Sign in to access all chart features, personalized analysis, and trading tools.
              </div>
              <Button
                size="sm"
        ...
        <DetailedTechnicalAnalysis symbol={symbol} interval={interval} />
          ...
          {!user ? (
            <Alert className="border-primary/30 bg-primary/5 mb-6">
              ...
              <AlertDescription className="flex flex-col sm:flex-row justify-between items-center gap-4">
                <p>Sign in to participate in our community forum.</p>
